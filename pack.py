...
d = LpVariable.dicts('d', [(i, j, k) for i in vars for j in vars for k in range(4)],
...
for i in vars:
    ...
    # sequence
    if i != vars[len(vars)-1]:
        model += (W*y[i] + x[i] <= W*y[i+1] + x[i+1])
    # containment
    model += (x[i] + w[i] <= W)
    # ratios
    if i % 2 != 1:
...
model += lpSum([2*y[i]+x[i] for i in vars])
